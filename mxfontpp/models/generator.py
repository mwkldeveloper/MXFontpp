...
class Generator(nn.Module):
    def __init__(self, C_in, C, C_out, style_enc, experts, emb_num, dec):
        ...
        for _key in self.feat_shape:
            ...
            self.fact_blocks[_key] = nn.ModuleList([nn.Sequential(nn.Conv2d(_feat_C, emb_num*_feat_C, 1, 1), nn.BatchNorm2d(emb_num*_feat_C), nn.LeakyReLU())
                                                    for _ in range(self.n_experts)])
            self.recon_blocks[_key] = nn.ModuleList([nn.Sequential(nn.Conv2d(emb_num*_feat_C, _feat_C, 1, 1), nn.BatchNorm2d(_feat_C), nn.LeakyReLU())
                                                    for _ in range(self.n_experts)])
